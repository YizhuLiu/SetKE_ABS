...
from model.rl_ import ActorCritic
from model.extract import PtrExtractSumm

from training import BasicTrainer
from rl_ import get_grad_fn
from rl_ import A2CPipeline
...
class RLDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        abs_sents = js_data['new_abs_sents']
        matched_abss = []
        for i in range(0,len(abs_sents),2):
            matched_abss.append(abs_sents[i])
        extract = js_data['extracted']
        return art_sents, matched_abss, extract
...
def build_batchers(batch_size):
    def coll(batch):
        ...
        extract = list(filter(bool, ext_batch))
        return art_sents, abs_sents, extract
    loader = DataLoader(
