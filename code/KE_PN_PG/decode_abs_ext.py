...
MAX_ABS_NUM = 5  # need to set max sentences to extract for non-RL extractor


def decode(save_path, abs_dir, ext_dir, split, batch_size, beam , max_len, cuda):
    start = time()
    # setup model
    if abs_dir is None:
    ...
    dec_log['rl'] = False
    dec_log['split'] = split
    dec_log['beam'] = beam  # greedy decoding only
    ...
    with torch.no_grad():
        for i_debug, raw_article_batch in enumerate(loader):
            ...
            for raw_art_sents in tokenized_article_batch:
                exts = extractor(raw_art_sents)
                ext_art = []
                ext_ind = []
                #ext_inds += [(len(ext_arts), len(exts.count(0)))]
                artL = len(ext_arts)
                ext_ = []
                for ext in exts:
                    if ext == 0:
                       if ext_art:
                          ext_arts.append(ext_art)
                          ext_.append(1)
                       ext_art = []
                       ext_ind = []
                    else:
                       #ext_ind += [(len(ext_art), len(ext_))]
                       #ext_art += list(map(lambda i: raw_art_sents[i], ext_))
                       ext_art += raw_art_sents[ext]
                if exts[-1] != 0 and ext_art:
                   ext_arts.append(ext_art)
                   ext_.append(1)
                ext_inds += [(artL, len(ext_))]
            #print(ext_arts,ext_inds)
            #exit(0)
            ...
            for j, n in ext_inds:
                ...
                #print(len(decoded_sents), decoded_sents)
                #exit(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beam', type=int, action='store', default=5,
                        help='beam size of faster decoding')
