...
from decoding_abs import Abstractor, Extractor, DecodeDataset
from decoding_abs import make_html_safe
    ...
    with torch.no_grad():
        for i_debug, raw_article_batch in enumerate(loader):
            ...
            for raw_art_sents in tokenized_article_batch:
                ...
                #print(ext,len(list(map(lambda i: raw_art_sents[i], ext))))
                #print(ext_arts,ext)#extractive art_sent and corresponding index
                #exit(0)
            dec_outs = abstractor(ext_arts)
            #print(dec_outs)
            #print(ext_inds)
            #print(i, i_debug, len(ext_inds), len(raw_article_batch))
            #print(ext_inds)
            #exit(0)
            ...
            for j, n in ext_inds:
                ...
                for k, dec_str in enumerate(decoded_sents):
                    #print(k,dec_str)
                    #if not os.path.exists(save_path+'/output_{}'.format(k)):
                    if not os.path.exists(save_path+'/output'):
                       os.makedirs(join(save_path, 'output'))
                    #   os.makedirs(join(save_path, 'output_{}'.format(k)))
