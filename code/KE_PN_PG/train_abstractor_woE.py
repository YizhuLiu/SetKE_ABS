...
class MatchDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        art_sents, abs_sents, extracts = (
            js_data['article'], js_data['new_abs_sents'], js_data['extracted'])
        ...
        for i in extracts:
            if i == 0:
               matched_arts.append(new_art_sent.strip())
               new_art_sent = ''
            else:
               new_art_sent = art_sents[i] + ' '
        #print(len(matched_arts), len(abs_sents))               
        matched_abss = []
        for i in range(0,len(abs_sents),2):
            matched_abss.append(abs_sents[i])

        return matched_arts, matched_abss[:len(matched_arts)]
...
def configure_training(opt, lr, clip_grad, lr_decay, batch_size):
    ...
    def criterion(logits, targets):
        return sequence_loss(logits, targets, nll, pad_idx=PAD,model='abs')
...
if __name__ == '__main__':
    ...
    # length limit
    parser.add_argument('--max_art', type=int, action='store', default=85,
