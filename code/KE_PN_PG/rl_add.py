...
def a2c_train_step(agent, abstractor, loader, opt, grad_fn,
    ...
    for inds, exts, arts, setR, summR, zero in zip(indices, ext_batch, art_batch, set_reward, summ_reward, zeros):
        #print(inds,exts,arts)
        #print(len(setR), len(zero))
        #exit()
        #inds = [1,2,0,4,0,1,0,25,0,1,0]
        #exts = [1,0,4,0,5,1,0,0]
        cand_inds = inds
        cand_exts = exts
        inds0 = []
        exts0 = []
        for e, index in enumerate(cand_inds):
            if index == 0:
               inds0.append(e)
        for e, index in enumerate(cand_exts):
            if index == 0:
               exts0.append(e)
        add = 0
        #print(inds0,exts0)
           
        for k in range(min(len(inds0),len(exts0))):
            if inds0[k] < exts0[k]: 
               add = exts0[k] - inds0[k]
               for z in range(len(zero)):
                   if zero[z] >= exts0[k]:
                      zero[z] += add
               for n in range(inds0[k], inds0[k]+add):
                   cand_inds.insert(n+1,0)
               new_inds0 = list(map(lambda x:x+add,inds0[k:])) 
               inds0 = inds0[:k] + new_inds0
            elif exts0[k] < inds0[k]:
               add = inds0[k] - exts0[k]
               for n in range(exts0[k], exts0[k]+add):
                   cand_exts.insert(n+1,0)
               new_exts0 = list(map(lambda x:x+add,exts0[k:])) 
               exts0 = exts0[:k] + new_exts0
            #print(inds0,exts0)
        #print(inds,exts,zero)
        #exit()
        rs1 = []
        k = 0
        
        for j in range(min(len(inds)-1, len(exts), len(cand_inds)-1, len(cand_exts))):
            #print(set(inds) & set(exts))
            #if j in (set(inds0) & set(exts0)):
            if j in zero and k < len(setR):
               rs1.append(setR[k])
               k+=1
            else:
               rs1.append(compute_rouge_n(arts[inds[j]], arts[exts[j]],n=1))
        rs2 = [0 for _ in range(max(0, len(inds)-1-len(exts)))]  
        stop = metrics(gt=exts, pred=inds, metrics_map=['MAP'])[0]
        rs3 = [stop_coeff*summR]
        rs = rs1 + rs2 + rs3
        '''
        rs = ([stop_reward_fn(arts[inds[j].item()], arts[exts[j]])
              for j in range(min(len(inds)-1, len(exts)))]
              + [0 for _ in range(max(0, len(inds)-1-len(exts)))]
              + [stop_coeff*(metrics(gt=exts, pred=inds, metrics_map=['MAP'])[0])])
        '''
        assert len(rs) == len(inds)
